# 肺炎 X 光影像辨識｜Pneumonia Detection

## 專題說明
使用胸部 X 光影像辨識正常與肺炎兩種類別，分別以自建 CNN 與 ResNet50 遷移學習實作，比較兩種方法的差異。</br>
...
## 分析流程
1. v1：自建 CNN，實驗不同 epochs 找最佳訓練輪數
2. v2：載入 ResNet50 預訓練模型，分階段訓練與微調
...
## 臨床觀點
FN（假陰性：肺炎被預測為正常）是最危險的錯誤，可能導致延誤治療。</br>
實際臨床應用仍需搭配醫師判斷，不可單獨作為診斷依據。
