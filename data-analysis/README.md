# Data Analysis Projects

紀錄跟隨課程完成的資料分析專題。

## 專題列表

1. [兩百個國家、兩百年、四分鐘](./01-gapminder_clone) - 復刻 Hans Rosling 經典資料視覺化，
   呈現兩百年間各國人均GDP與預期壽命的動態變化<br>
   👉 [互動式瀏覽](https://ouziling890620.github.io/Data-science-portfolio/data-analysis/01-gapminder_clone/gapminder_clone.html)

2. [拿破崙征俄戰爭](./02-minard_clone) - 復刻 Charles Minard 經典資料視覺化，
   在同一張圖呈現1812年拿破崙大軍的行軍路線、人數變化與氣溫下降<br>
   👉 [成品圖片](https://github.com/ouziling890620/Data-science-portfolio/blob/main/data-analysis/02-minard_clone/minard_clone.png)

3. [大疫世代](./03-covid_19_pandemic) - 使用CSSE at Johns Hopkins資料，
   建立含地理地圖與時間序列的Covid-19疫情儀表板<br>
   👉 [線上互動瀏覽（Hugging Face Spaces部署）](https://huggingface.co/spaces/ouziling890620/covid_19)

6. [找出章魚里](./06-taiwan_presidential_election_2024) - 以餘弦相似度分析 2024 總統大選村里得票結構，
   練習正規化資料庫設計、向量相似度計算、gradio 互動介面<br>
   👉 [gradio 查詢介面程式碼](https://github.com/ouziling890620/Data-science-portfolio/blob/main/data-analysis/06-taiwan_presidential_election_2024/app.py)

7. [資料科學家的工具箱](./07-data_scientists_toolbox) - 分析 2020-2022 年 Kaggle ML & DS Survey 普查資料，
   練習跨年度資料整併、SQL JOIN、matplotlib 跨年份比較視覺化<br>
   👉 [成品圖片](https://github.com/ouziling890620/Data-science-portfolio/blob/main/data-analysis/07-data_scientists_toolbox/data_science_job_titles.png)